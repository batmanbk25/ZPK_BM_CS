*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_BM_CSTR_C
*   generation date: 21.10.2022 at 18:00:44
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVI_BM_CSTR_C       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_BM_CSTR_C'
         EXPORTING
              FCODE          = READ
...
CALL FUNCTION 'VIEWPROC_ZVI_BM_CSTR_C'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_BM_CSTR_C-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_BM_CSTR_C_EXTRACT
TOTAL = ZVI_BM_CSTR_C_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_BM_CSTR_C-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_BM_CSTR_C'
...
TOTAL = ZVI_BM_CSTR_C_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_BM_CSTR_C-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_BM_CSTR_C-UPD_FLAG NE SPACE.
